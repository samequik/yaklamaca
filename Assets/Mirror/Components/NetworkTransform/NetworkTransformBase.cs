...
namespace Mirror
{
    ...
    public abstract class NetworkTransformBase : NetworkBehaviour
    {
        ...
        {
            // YEREL YAMA (Yakalamaca) — Mirror güncellenirse bu satır kaybolur.
            //
            // Reset() editörde AddComponent anında çalışıyor; Awake() de
            // OnValidate() de henüz hiç çalışmamış oluyor, dolayısıyla target
            // null. Aşağıdaki ResetState() -> GetPosition() bu yüzden
            // NullReferenceException atıyor ve istisna Reset()'i yarıda kestiği
            // için altındaki iki satır hiç çalışmıyor: syncInterval 0.05 yerine
            // 0'da kalıyor, yani transform istenenden ~3 kat sık gidiyor.
            //
            // Awake() zaten aynı gerekçeyle Configure() çağırıyor
            // ("sometimes OnValidate() doesn't run before launching a project").
            Configure();
